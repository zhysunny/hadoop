...
abstract class TaskRunner extends Thread {
    ...
    public final void run() {
        try {
            ...
            LOGGER.error("FSError", e);
            try {
                tracker.fsError(e.getMessage());
            } catch (IOException ie) {
                LOGGER.error(t.getTaskId() + " reporting FSError", ie);
            }
        } catch (Throwable throwable) {
            LOGGER.error(t.getTaskId() + " Child Error", throwable);
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            throwable.printStackTrace(new PrintStream(baos));
            try {
                tracker.reportDiagnosticInfo(t.getTaskId(), baos.toString());
            } catch (IOException e) {
                LOGGER.error(t.getTaskId() + " Reporting Diagnostics", e);
            }
        } finally {
    ...
    private String handleDeprecatedHeapSize(String javaOpts,
        ...
        LOGGER.warn("mapred.child.heap.size is deprecated. Use " +
    ...
    private void logStream(InputStream output) {
        try {
            ...
            while ((line = in.readLine()) != null) {
                LOGGER.info(t.getTaskId() + " " + line);
            }
        } catch (IOException e) {
            LOGGER.error(t.getTaskId() + " Error reading child output", e);
        } finally {
            try {
                output.close();
            } catch (IOException e) {
                LOGGER.error(t.getTaskId() + " Error closing child output", e);
            }
        }
    }
}
